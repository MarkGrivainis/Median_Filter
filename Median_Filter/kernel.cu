...
int main(int argc, char* argv[])
{
	...
	int dim = 3;
	if (argc < 3) {
		printf("The program requires you to specify the bin dimension and filter radius \n using bin size - 1024 \n filter dim - 3\n\n");
	}
	else
	{
		try{
			bin_dim = std::stoi(argv[1]);
		}
	...
	ReadWrite::LoadData_s(grid_p, name);
	printf("--- OMP Binning ---\n\n");
...
cudaError_t median_filter(Grid &output, const Grid &input, const int filter_radius, int padding_type)
{
	...
	cudaThreadSetLimit(cudaLimitMallocHeapSize, (8 + 2 * filter_radius)*(8 + 2 * filter_radius) * filter_radius*filter_radius * sizeof(int));
...
//		threads = dim3(BLOCK_H, BLOCK_W);
	...
	*/cudaEventRecord(start);
	...
	printf("--- 5x5 Padding Quick Select \t: %-12.5f sec\n", milliseconds/1000);
